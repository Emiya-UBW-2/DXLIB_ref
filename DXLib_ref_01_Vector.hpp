...
namespace DXLibRef {
	...
	public:
		...
		VECTOR get(void) const noexcept { return DxLib::VGet(this->x, this->y, 0.f); }				//変換
		static const Vector2DX vget(float x, float y) noexcept { return DxLib::VGet(x, y, 0.f); }	//入力
		//
		...
		//
		float magnitude(void) const noexcept { return DxLib::VSize(this->get()); }				//サイズ
		Vector2DX normalized(void) const noexcept { return DxLib::VNorm(this->get()); }		//正規化
		float sqrMagnitude(void) const noexcept { return DxLib::VSquareSize(this->get()); }		//サイズ
		//
