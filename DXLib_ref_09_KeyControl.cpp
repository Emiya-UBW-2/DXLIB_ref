...
namespace DXLib_ref {
	...
	void PadControl::Execute(void) noexcept {
		...
		//コントロールタイプ決定
		{
			ControlType PrevControlType = m_ControlType;
			m_ControlType = ControlType::PC;
			if (GetJoypadNum() > 0) {
			...
			if (PrevControlType != m_ControlType) {
				PrevControlType = m_ControlType;
				m_IsUpdate = true;
			}
		}
		//タイプに合わせた操作
		...
		case DXLib_ref::ControlType::PC:
		{
			...
			//右スティック
			{
				if (!DrawParts->IsPause()) {
					...
					Look_XradAdd = std::clamp((float)(mx - DrawParts->m_DispXSize / 2), -180.f, 180.f);
					Look_YradAdd = std::clamp(-(float)(my - DrawParts->m_DispYSize / 2), -180.f, 180.f);
				}
				else {
					SetMouseDispFlag(TRUE);
					Look_XradAdd = 0.f;
